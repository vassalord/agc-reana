...
def set_pyhf_backend():
    ...
    try:
        ...
        pyhf.set_backend(backend)
        print("[WARN] Old pyhf: 'default_minimizer' not supported; using SciPy.")
...
def try_import_agc_rebinning_module():
    ...
    try:
        ...
        print(f"[WARN] Failed to import AGC rebinning: {e}")
        return None
    ...
    if hist_mod is not None and agc_rebin is not None:
        try:
            rebinning_router = agc_rebin.get_cabinetry_rebinning_router(
                cfg, rebinning=slice(110j, None, hist_mod.rebin(merge_factor))
            )
...
def robust_observed_limit_scan(model, data, cls_target=0.05, mu_min=0.0, mu_max=50.0, n_steps=200):
    ...
    for i in range(n_steps + 1):
        ...
        try:
            cls = float(pyhf.infer.hypotest(mu, data, model, init_pars=init_pars, par_bounds=par_bounds))
...
def main():
    ...
    import math as _m
    lo = max(0.0, float(lo)) if _m.isfinite(lo) else 0.0
    hi = float(hi) if _m.isfinite(hi) and hi > 0 else 100.0
    bracket = (lo, hi)
    ...
    try:
        ...
        res = cabinetry.fit.limit(
            model, data, poi_name=poi, bracket=bracket, strategy=1, maxiter=20000,
        )
    ...
    try:
        ...
        workspace.save(ws2, "workspace.json")
        ...
        observed = getattr(res, "observed_limit", getattr(res, "upper_limit", None))
        expected = getattr(res, "expected_limit", None)
        write_results(poi, observed, expected, used_rebin2, "cabinetry.fit.limit (merge 4->1)", ok=True)
        return
    except Exception as e:
    ...
    write_results(poi, mu_obs, None, used_rebinning, "fallback brute CLs scan (observed only); expected bands not computed", ok=False)
